...
function sysmsg($msg = '未知的异常',$die = true) {
    ...
    <body id="error-page">
        <?php echo '<h3>站点提示信息</h3>'; echo $msg; ?>
    </body>
    </html>
<?php
if ($die == true) {
...
if ($userrow['stype']==1 and $srow['addmoney'] >= 0.1) {
$BizContent = array(
	'out_biz_no' => $srow['trade_no'], //商户转账唯一订单号
	'payee_type' => 'ALIPAY_LOGONID', //收款方账户类型
	'payee_account' => $srow['alipayid'], //收款方账户$payee_account
	'amount' => $srow['addmoney'] ,//转账金额$value
	'payer_show_name' => $conf['bz'], //付款方显示姓名
	'payee_real_name' => $srow['username'], //收款方真实姓名$payee_real_name
);
...
$DB->exec("update `pay_alisettle` set `transfer_result`='".$result->$responseNode->order_id."',`transfer_date`='".$result->$responseNode->pay_date."' where `out_trade_no`='".$srow['trade_no']."'");
$DB->exec("update `pay_alisettle` set `transfer_status`='1',`status`='1',`time`='{$date}' where out_trade_no='".$srow['trade_no']."' limit 1");
} else{
$DB->exec("update `pay_alisettle` set `transfer_result`='未知错误' where `out_trade_no`='".$srow['trade_no']."'");
$DB->exec("update `pay_alisettle` set `transfer_status`='2',`status`='2' where out_trade_no='".$srow['trade_no']."' limit 1");
$DB->exec("update pay_user set money=money+{$srow['addmoney']} where id='{$srow['pid']}'");
}
}else{
	$DB->exec("update pay_user set money=money+{$srow['addmoney']} where id='{$srow['pid']}'");
}
