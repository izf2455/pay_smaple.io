<?php
/*天方夜谭支付财付通自动补单-code's by 天方夜谭！
说明：本页面用于天方夜谭支付系统对接第三方易支付时，请求支付接口订单列表，同步未通知到本站的订单，防止漏单。
温馨提示：监控频率建议5分钟一次，千万不要监控太快或使用多节点监控，否则可能会被支付接口自动屏蔽IP地址！
*/
include("../common.php");
$key = isset($_GET['key']) ? $_GET['key'] : null;
if (!($conf['cron_key'] && $key && $key == $conf['cron_key'])) {
    exit('天方夜谭支付系统提醒您：您的监控识别码不正确，请前往系统后台查看或修改！');
}
...
if($arr['code']==1){
  ...
  processOrder($srow);
				echo '天方夜谭支付程序非常负责的告诉您，已成功补单的有：'.$out_trade_no.'<br/>';
			}
		}
	}
	exit('ok');
